...
// TestAV1CapturedPacketsReplay loads captured RTP packets and tests depacketization.
// To capture packets, set AV1CaptureFile before running the transcoder with AV1 input.
func TestAV1CapturedPacketsReplay(t *testing.T) {
	// Try to load captured packets from common locations
	captureFiles := []string{
		"testdata/av1_capture.bin",
		"/tmp/av1_capture.bin",
	}
	...
	for _, f := range captureFiles {
		packets, loadErr = AV1LoadCapturedPackets(f)
		if loadErr == nil && len(packets) > 0 {
			loadedFile = f
			break
		}
	}
	...
	if len(packets) == 0 {
		t.Skip("No captured packets found. To capture: set media.AV1CaptureFile = \"/tmp/av1_capture.bin\" before running transcoder")
	}

	t.Logf("Loaded %d packets from %s", len(packets), loadedFile)
	...
	for i, pktData := range packets {
		frame, err := depacketizer.DepacketizeBytes(pktData)
		if err != nil {
			t.Logf("Packet %d: depacketize error: %v", i, err)
			continue
		}
		if frame != nil {
			frames = append(frames, frame)
			t.Logf("Packet %d: got frame %d, %d bytes, keyframe=%v",
				i, len(frames), len(frame.Data), frame.FrameType == FrameTypeKey)
		}
	}

	t.Logf("Depacketized %d packets into %d frames", len(packets), len(frames))

	if len(frames) == 0 {
		t.Fatal("No frames depacketized")
	}

	// Verify first frame has valid OBU structure
	for i, frame := range frames {
		if len(frame.Data) < 2 {
			t.Errorf("Frame %d too short: %d bytes", i, len(frame.Data))
			continue
		}
		// Should start with Temporal Delimiter (0x12, 0x00)
		if frame.Data[0] != 0x12 || frame.Data[1] != 0x00 {
			t.Logf("Frame %d: unexpected start bytes %02x %02x (expected 12 00 for TD)",
